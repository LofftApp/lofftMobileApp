  ...
  endpoints: builder => ({
    getChatrooms: builder.query<ChatroomsState, void>({
      ...
      providesTags: result =>
        result
          ? [
              ...result.chatrooms.map(({id}) => ({type: 'Chat', id} as const)),
              {type: 'Chat', id: 'LIST'},
            ]
          : [{type: 'Chat', id: 'LIST'}],
    }),
    getChatroombyId: builder.query<MessagesState, number>({
    ...
    createMessage: builder.mutation<void, {id: number; content: string}>({
      query: ({id, content}) => ({
